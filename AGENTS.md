...
- Planning artifacts in `.planning/` (PROJECT.md, ROADMAP.md, REQUIREMENTS.md, STATE.md, per-milestone dirs) are GSD-era history. Live workflow (rebuilt 2026-08-27): pick the next thing -> build -> test once -> commit, push, tick the box; /land is the two-minute mechanical close; /check on demand only.
