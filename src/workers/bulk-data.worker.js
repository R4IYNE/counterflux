...
// Phase 8 Plan 3 — mirror of main-thread v9 (precons_cache additive).
// Per D-26 + PITFALLS §1: worker only touches `cards` + `meta`, but every
// connection to the same IDB must declare the full schema chain so Dexie's
// schema-version compare passes. No .upgrade() callback — v9 is additive.
db.version(9).stores({
  collection: 'id, scryfall_id, category, foil, user_id, updated_at, synced_at, [scryfall_id+foil], [scryfall_id+category]',
  decks: 'id, name, format, user_id, updated_at, synced_at',
  deck_cards: 'id, deck_id, scryfall_id, user_id, updated_at, synced_at, [deck_id+scryfall_id]',
