...
- [ ] **PERF-01**: Web Vitals instrumentation (`web-vitals` 5.2.x) records LCP, FID, CLS, TTI, FCP on every page load; metrics visible in console in dev mode
- [ ] **PERF-02**: Lighthouse CI (`@lhci/cli`) added as dev dependency with a `npm run perf` script that produces a desktop-preset report against `vite preview`
- [ ] **PERF-03**: Baseline performance report committed to `.planning/` capturing current v1.0 numbers; target TTI/LCP set based on measured baseline
...
- [ ] **COLLECT-01**: "Query database" results in add-card modal never show mana cost (audit confirms removed if previously present)
...
- [ ] **COLLECT-03**: Card image preview renders in the card entry selection dropdown (thumbnail visible alongside name before selection)
...
- [ ] **COLLECT-05**: Mass entry terminal has a visible X close button in the header wired to the existing `discard()` method
