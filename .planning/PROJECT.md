...
- ✓ Performance optimisation (PERF-04) — v1.1 meets Web Vitals budget. LCP 6.1s → 2.49s (−59%) via bfcache handlers (D-09), shimmer composability fix (D-08), splash-to-migration-only + streaming UI + topbar bulk-data pill (D-04/05/06) + honest empty-state gating, Vite `manualChunks` CSS/JS split, `font-display: swap` + Syne preload, Pitfall 15 cache-bust recovery (`vite:preloadError` + `Cache-Control: no-cache`), bundle-budget enforcement script, and (the actual LCP win) static paint-critical `<h1>` in initial HTML with auth-wall decorating pre-existing DOM. CLS targeted fixes (Plan 4) skipped with audit trail — already under target. CI soft-gate via `@lhci/cli` warns-not-blocks on PRs. Final: LCP 2.49s / FCP 0.4s / CLS 0.059 / Perf 86 — all Web Vitals `Good`. v1.1 Phase 13
...
---
*Last updated: 2026-04-22 — Phase 13 (Performance Optimisation, conditional) complete; shipped PERF-04 across 5 plans (Plan 4 skipped via D-07 audit-driven skip path, CLS already under target). Branch B taken after Plan 1 re-measurement found LCP 6.1s gap. Final v1.1 Lighthouse (headless desktop): LCP 2.49s / FCP 0.4s / CLS 0.059 / Perf score 86 — all Web Vitals `Good`, PERF-04 formally closed in `13-PERF-SIGNOFF.md`. The whole LCP win concentrated in Plan 5 Task 6's static paint-critical `<h1>` moved into initial HTML (auth-wall now decorates pre-existing DOM instead of creating at runtime); infrastructure (bundle split, cache-bust recovery, budgets) shipped alongside. CI soft-gate via `@lhci/cli` warns on PRs without blocking. Test suite 1009 pass / 0 fail across 115 files. Two live-environment UAT items persist (soft-gate on real PR; Vercel `Cache-Control: no-cache` header emission) — captured in `13-HUMAN-UAT.md`. Milestone v1.1 Second Sunrise complete. Next: `/gsd:complete-milestone` to archive, or start v1.2 scoping.*
